...
def load_reference_satker():
    ...
    try:
        url = (
            ...
            "ameernoor/kinerjakeuangansatkerbtastreamlit/main/templates/"
...
def process_excel_file_kppn(uploaded_file, year, detected_month=None):
    try:
        ...
        if "df_upload" not in st.session_state:
            st.session_state.df_upload = pd.read_excel(uploaded_file)

        df_raw = st.session_state.df_upload
...
def save_file_to_github(content_bytes, filename, folder):
    ...
    try:
        ...
        repo.update_file(existing.path, f"Update {filename}", content_bytes, existing.sha, branch="main")
    except Exception:
        # 3️⃣ jika folder tidak ada → buat file pertama
        repo.create_file(path, f"Create {filename}", content_bytes, branch="main")
...
def push_to_github(file_bytes, repo_path, repo_name, token, commit_message):
    ...
    try:
        ...
        try:
            ...
            repo.update_file(contents.path, commit_message, file_bytes, contents.sha, branch="main")
            st.success(f"✅ File {repo_path} berhasil diupdate di GitHub")
        except Exception as e_inner:
            # Jika file belum ada atau path salah, buat baru
            repo.create_file(repo_path, commit_message, file_bytes, branch="main")
        ...
        if uploaded_cms:
            ...
            with st.spinner("Memproses Data CMS..."):
                ...
                for sheet in xls.sheet_names:
                    ...
                    # ===============================
                    # 🔥 AUTO DETECT HEADER (SMART)
                    # ===============================
                    header_row = None
                    KEYWORDS = ["KPPN", "SATKER", "TRANSAKSI", "JUMLAH", "NAMA"]
                    ...
                    # ===============================
                    # BUANG BARIS SAMPAH (TW3 FIX)
                    # ===============================
                    df = df.dropna(how="all")
                    df = df[~df.iloc[:,0].astype(str).str.contains("REKAP|KATEGORI", case=False, na=False)]
                    ...
                    for col in df.columns:
                        ...
                        if test.notna().sum() > 5:
                            col_satker = col
                            df[col] = test.fillna("").str.zfill(6)
                            break
                ...
                cms_master = st.session_state.get("cms_master", pd.DataFrame())
        ...
        if not st.session_state.data_storage:
            ...
            if st.button("🗑️ Hapus Data IKPA Satker", type="primary") and confirm_delete:
                try:
                    ...
                    repo.delete_file(contents.path, f"Delete {filename}", contents.sha, branch="main")
        ...
        if not st.session_state.get("DATA_DIPA_by_year"):
            ...
            if st.button("🗑️ Hapus Data DIPA Ini", type="primary", key="btn_delete_dipa") and confirm_delete_dipa:
                try:
                    ...
                    repo.delete_file(contents.path, f"Delete {filename_dipa}", contents.sha, branch="main")
        ...
        if "cms_master" not in st.session_state or st.session_state.cms_master.empty:
            ...
            if st.button("🗑️ Hapus Data CMS", key="btn_delete_cms") and confirm_delete_cms:
                ...
                try:
                    ...
                    try:
                        file = repo.get_contents(file_path)
                        repo.delete_file(file.path, f"Delete {file_name}", file.sha, branch="main")
                    except:
                        pass
